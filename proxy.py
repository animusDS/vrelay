...
class Proxy:
    ...
    def Start(self):
        ...
        self.serverMonitorThread = threading.Thread(
            target=self.ServerMonitor, daemon=True)
        self.serverMonitorThread.start()
        self.Connect()

    def Connect(self):
        # connect sockets first
        self.client.ConnectRemote(
            self.client.remoteHostAddr, self.client.remoteHostPort)
...
def main():
    ...
    with open("bin/BulletDictionary.pkl", "rb") as f:
        ...
        print("[Initializer]: Deserialized {} enemies.".format(
            len(set([x[0] for x in bulletDictionary.keys()]))))
        print("[Initializer]: Deserialized {} bullets.".format(
            len(bulletDictionary)))
    ...
    with open("bin/AoeDictionary.pkl", "rb") as f:
        ...
        print("[Initializer]: Deserialized {} AOEs.".format(
            sum({y: len(aoeDictionary[y]) for y in aoeDictionary}.values())))

    print("[Initializer]: Starting proxy...")
    client = Client(plugins, bulletDictionary, nameDictionary,
                    tileDictionary, aoeDictionary)
